 ...
 */
package com.conceptualgraph.charger.layout;
 ...
 * together.
 * <p>
 * Fine tuning of the layout parameters is crucial to the effective use of the algorithm. 
 * See comments on each of the layout parameters to better understand how they 
 * work.
 * </p>
 * 
 * <p> Future tweaks: 
...
//    private static double leftPressure = 1.0;
//    private static double upPressure = 1.0;
    private boolean applyUpLeftPressure = true;
    
    private double pressureScale = 0.2;    // proportion to squeeze up or left; multiplier for upleft force
    /**
...
//            if ( verbose ) Global.info( "node " + node.getTextLabel() + "  after pos " + node.getCenter() );
     ...
     * graph, just in case there's an error; simply updates the nodes and edges
     * collections.
     *
     * @see #copyToGraph()
     */
    public boolean performLayout() {
        Global.info( "Spring layout starting with " + nodes.size() + " nodes and " + edges.size() + " edges..." );
        if ( nodes.size() == 0 ) {
...
//              showAllForces( "at start...");
        ...
        return makeNewPositions( applyUpLeftPressure );
    }
...
//        layout.setMaxBoundsAvailable( this.inferCurrentDisplayRect( g ) );
//        layout.setMaxBoundsAvailable( Util.make2DDouble( g.getBounds()) );
...
//            if ( verbose ) 
     ...
     * @param upLeftPressure if true, then an additional "pressure" is applied to 
     * force nodes upward and to the left. This should help layouts from getting too spread out.
...
//        Point2D.Double upLeftPressureForce = new Point2D.Double( 0.0, 0.0 );
        double leftPressureValue = 0.0;
        double upPressureValue = 0.0;

        // Now that we have the forces for every node, apply them and get new displacements and positions.
        for ( GNode gn : nodes ) {
            if ( upLeftPressure  )  {
                leftPressureValue = -1 * nodeForces.get( gn ).x * pressureScale;
                upPressureValue = -1 * nodeForces.get( gn ).y * pressureScale;
            }
//                    showDisplacements( "before adding netforce ");
            nodeDisplacements.get( gn ).x += nodeForces.get( gn ).x * DAMPING_FOR_DISPLACEMENTS / mass( gn ) + leftPressureValue;
            nodePositions.get( gn ).x += nodeDisplacements.get( gn ).x;
            nodeDisplacements.get( gn ).y += nodeForces.get( gn ).y * DAMPING_FOR_DISPLACEMENTS / mass( gn ) + upPressureValue;
...
//        }
     ...
     * @param force x and y values to be added to the force, multiplied by the "mass" of the node.
     * @param node The node whose force is to be altered
     */
...
//            Global.info( "after copyToGraph, graph is\n" + CGXGenerator.generateXML( graph ));
    ...
    public static String showForce( Point2D.Double p ) {
        ...
        return "Total: " + nformat.format( force ) + ", x: " + nformat.format( p.x )
...
//        for ( Point2D.Double pos : nodePositions.values() ) {
    ...
    private Rectangle2D.Double inferCurrentDisplayRect( GNode node ) {
        ...
        if ( center == null ) {
            center = General.make2DDouble( node.getCenter() );
        }
//        if ( center == null ) {
...
//        }
    ...
    public double getClippedLength( GNode node1, GNode node2 ) {
        Point2D.Double pos1 = General.make2DDouble( nodePositions.get( node1 ) );
        Point2D.Double pos2 = General.make2DDouble( nodePositions.get( node2 ) );
        ...
        Point2D.Double clipPointTo =
                General.make2DDouble( GEdge.findClippingPoint( pos1, pos2, node2Rect ) );
        Point2D.Double clipPointFrom =
                General.make2DDouble( GEdge.findClippingPoint( pos2, pos1, node1Rect ) );
...
//        Point2D.Double clipPointFrom = GEdge.findClippingPoint( clipPointTo, (float)nodePositions.get(node1), node1.getShape());
    ...
    public void setMaxBoundsAvailable( Rectangle rect ) {
        SpringGraphLayout.this.setMaxBoundsAvailable( new Rectangle2D.Double( rect.x, rect.y, rect.width, rect.height ) );
    }

    /** Set the maximum space allowed in laying out the graph. May increase or decrease the 
     * space it takes to display the laid out graph.
     * @param maxBoundsAvailable 
     */
