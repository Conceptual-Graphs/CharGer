 ...
 * substitute.
 *
 * @see com.conceptualgraph.chargerlib.WindowManager
 */
public class ObjectLocator {
    ...
    public static Object getObject( File f, GraphObjectID id, Class c, boolean makeAvailable ) {
        ...
        if ( mw == null ) {
            Global.info( "window for " + f.getAbsolutePath() + " not found." );
            if ( !makeAvailable ) {
                return null;		// for now
            } else {
                // Open the file here, and assign its window to mw
                if ( General.getFileExtension( f.getAbsolutePath() ).equalsIgnoreCase( Global.ChargerFileExtension ) ) {
                    String name = Global.openGraphInNewFrame( f.getAbsolutePath() );
                    mw = WindowManager.getWindowFromFile( f );
                } else {
                    Global.warning( "Don't know how to open file " + f.getAbsolutePath() );
                }
            }
        }
        Global.info( "window for " + f.getAbsolutePath() + " now open." );
     ...
     * <code>file:/Volumes/hsd/Research/CGs/catonmat.cgx#com.conceptualgraph.charger.obj.Concept-12345667</code>
    ...
    public String toURI() {
        try {
            ...
            s.append( "-" );
            s.append( _ID  );
            Global.info( "URI created is " + s.toString() );
            return s.toString();
        }
    }
    ...
    public static ObjectLocator parseURI( String uri ) {
        try {
            ...
            Global.warning( "Illegal argument: Object locator tried to parse \""
                    + uri + "\". " + e.getMessage() );
            return null;
        } catch ( java.net.URISyntaxException se ) {
            Global.warning( "URI Syntax: Object locator tried to parse \""
                    + uri + "\". " + se.getMessage() );
            return null;
        } catch ( NullPointerException ne ) {
            Global.warning( "Null pointer: Object locator tried to parse \""
                    + uri + "\". " + ne.getMessage() );
            return null;
        } catch ( ClassNotFoundException ce ) {
            Global.warning( "Class not found: Object locator tried to parse \"" + uri + "\". " + ce.getMessage() );
            return null;
        }
    }
}
