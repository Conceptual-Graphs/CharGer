package com.conceptualgraph.charger.util;
...
//        cb.getEditor().getEditorComponent().setMinimumSize( cb.getSize() );
    ...
    public static Rectangle2D.Double unionDisplayRects( ArrayList nodeList ) {
        ...
        while ( iter.hasNext() ) {
            go = (GraphObject)iter.next();
            if ( r == null ) {
                r = go.getDisplayRect(); //Util.make2DDouble( go.displayRect );
            } else {
                r.add(  go.getDisplayRect() );
            }
        }
        return r;
    }
...
//                          Global.info( "Component number " + cnum + " has " + nodeLists.get( cnum ).size() + " nodes.");
    ...
    static public Point2D.Double getCenterPoint( ArrayList<GNode> nodes ) {
        ...
        Rectangle2D.Double rect = General.make2DDouble( nodes.get( 0 ).getDisplayRect() );
...
//            if ( node instanceof Graph ) continue;
     ...
     * Checks to make sure each edge is owned by at least one of the graphs that own its endpoints.
     * Checks to make sure all objects are owned by the graph in which they are nested.
     * @return true if all checks pass without problems; false otherwise (errors are written to
     * the console 
     * @see General#error(java.lang.String) 
     */
    static public boolean verifyIntegrityOfGraph( Graph g ) {
        boolean ok = true;
        GraphObjectID owner = g.objectID;
        ShallowIterator iter = new ShallowIterator( g );
        while ( iter.hasNext() ) {
            GraphObject go  = iter.next();
            if ( ! go.getOwnerGraph().equals( g )) {
                ok = false;
                General.error( "verifyIntegrityOfGraph: " + go.getClass().getSimpleName() + " " + go.toString() 
                        + " has owner " + go.getOwnerGraph().objectID + "; should have owner " + owner);
            }
            if ( go instanceof GEdge ) {
                GEdge edge = (GEdge)go;
                
                if ( edge.fromObj.getOwnerGraph() != g ) {
                    ok = false;
                    General.error( "verifyIntegrityOfGraph: " + edge.getClass().getSimpleName() + " from obj " 
                        + edge.fromObj.toString() 
                        + " has owner " + edge.fromObj.getOwnerGraph().objectID + "; should have owner " + owner);

                }
                if ( edge.toObj.getOwnerGraph() != g ) {
                    ok = false;
                    General.error( "verifyIntegrityOfGraph: " + edge.getClass().getSimpleName() + " to obj " 
                        + edge.toObj.toString() 
                        + " has owner " + edge.toObj.getOwnerGraph().objectID + "; should have owner " + owner);

                }
            }
            if ( go instanceof Graph ) {
                ok = verifyIntegrityOfGraph( (Graph)go ) && ok;
            }
        }
        return ok;
    }
    ...
    public static void showMessageDialog( Component parentComponent, Object message ) {
        JOptionPane.showMessageDialog( parentComponent, message, 
                "CharGer Message", JOptionPane.INFORMATION_MESSAGE, Global.applicationIcon );
    }
}
