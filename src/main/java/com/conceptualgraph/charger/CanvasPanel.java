...
//        createLabelChooserOLD();
        ...
        // CR-2001 11-09-19 hsd needed to explicitly make them null.
...
//        if ( source == textLabelEditField ) {
    ...
    public synchronized void paintComponent( Graphics g ) {
        ...
        if ( ef.selectionIsBeingDragged ) {
            ...
            Iterator<GraphObject> iter = ef.EFSelectedNodes.iterator();
            ...
            while ( iter.hasNext() ) {
                ...
                if ( go.myKind != GraphObject.Kind.GEDGE ) // omit GEdges for showing
                {
                    Rectangle2D.Double r =
                            new Rectangle2D.Double( go.getDisplayRect().x + transx, go.getDisplayRect().y + transy,
     ...
     */
    public void reset() {
        /*f ( userEditedGraphObject != null)  closeTextEditors(); */
...
//            nodeEditingDialog.setMinimumSize( textLabelEditField.getWidth() + 10, textLabelEditField.getHeight() + 50 );
    ...
    public void userFinishedEditingText( String userEditedLabel ) {
        ...
        // TODO: Check referent for invalid characters?
        ...
        if ( !oldTextLabel.equals( userEditedLabel ) ) {
            try {
                // Note that we don't call forgetObject, because we're just changing the referent
            ...
            if ( userEditedGraphObject instanceof GNode ) {
                ...
                UserHistoryRecord he = new UserHistoryRecord( userEditedGraphObject, System.getProperty( "user.name") );
                he.appendDescription( "User modified object. Label " + userEditedGraphObject.getTextLabel());

                userEditedGraphObject.addHistoryRecord( he );
