...
import com.conceptualgraph.chargerlib.GenericTextFrame;
import com.conceptualgraph.kb.matching.AbstractTupleMatcher;
import com.conceptualgraph.kb.matching.BinaryRelationMatch;
...
public class OperManager {
    ...
    public JMenuItem operationMakeTypeHierarchy = new JMenuItem( com.conceptualgraph.kb.ConceptManager.makeTypeHierarchyAction );
    ...
    private void makeMenus() {
        ...
        if ( Global.wordnetEnabled ) {
            ef.emgr.makeNewMenuItem( ef.operateMenu, com.conceptualgraph.charger.Global.strs( "AttachOntologyLabel" ), 0 );
            ...
            ef.emgr.makeNewMenuItem( ef.operateMenu, com.conceptualgraph.charger.Global.strs( "DeleteOntologyLabel" ), 0 );
    ...
    public void setMenuItems() {
        ...
        for ( int num = 0; num < ef.operateMenu.getItemCount(); num++ ) {
            ...
            if ( s.equals( MakeGenericCmdLabel ) ) {
                ...
                mi.setEnabled( ef.somethingHasBeenSelected );
            } else if ( s.equals( com.conceptualgraph.charger.Global.strs( "AttachOntologyLabel" ) ) ) {
                mi.setEnabled( ef.somethingHasBeenSelected );
            } else if ( s.equals( com.conceptualgraph.charger.Global.strs( "DeleteOntologyLabel" ) ) ) {
                mi.setEnabled( ef.somethingHasBeenSelected );
            }
        }
...
//                        && ( a.fromObj.objectID == noduplicates.get( k ).fromObj.objectID ) ) // duplicate arrow
        ...
        // Doesn't seem to have the effect I was expecting
        iter = new DeepIterator( g, GraphObject.Kind.GNODE );
        while ( iter.hasNext() ) {
    ...
    * */
    ...
    public void performActionAttachOntologyLabel( ArrayList objectList ) {
    	...
    	com.conceptualgraph.charger.gloss.wn.WordnetManager wnmgr = com.conceptualgraph.charger.gloss.wn.WordnetManager.getInstance(  );
        Iterator<GraphObject> iter = objectList.iterator();
        boolean changed = false;
        while ( iter.hasNext() ) {
            GraphObject go = iter.next();
            if ( go instanceof GNode ) {
            	com.conceptualgraph.charger.gloss.AbstractTypeDescriptor descr = ( (GNode)go ).getTypeDescriptor();
                //if ( descr == null )
                // NEED a parameter for com.conceptualgraph.charger preferences indicating whether to try generic
                // for now, force to try generic
                // NEED to pass type label as a phrase into this.
                changed = changed | wnmgr.attachDescriptor(
                		com.conceptualgraph.kb.ConceptManager.getSensesFromPhrase( ( (GNode)go ).getTypeLabel(), wnmgr ), (GNode)go );
                //else
                //	changed = changed | wnmgr.attachDescriptor(
    ...
    /* REMOVE-NOTIO public void performActionMatchToOpenGraphs( Graph g )
     ...
     Graph match = new com.conceptualgraph.charger.obj.Graph( null );
     //if ( matchng.getContextGraph() != null ) matchng = matchng.getContextGraph();
     match = NotioTrans.notioGraphToCharGer( matchng, new com.conceptualgraph.charger.obj.Graph( null ) );
    ...
    public void performActionMaxJoinWithOpenGraphs( Graph g ) {
        ...
        if ( joinMade ) {
            ...
            if ( newef != null ) {
                ...
                }			    //Graph join = NotioTrans.notioGraphToCharGer( joinGraph, new com.conceptualgraph.charger.obj.Graph( null ) );
    ...
    public void performActionMakeGeneric() {
        ...
        if ( ef.somethingHasBeenSelected ) {
            ...
            for ( int k = 0; k < ef.EFSelectedObjects.size(); k++ ) {
                GraphObject go = ef.EFSelectedObjects.get( k );
     ...
     * @param g
     */
    public void performActionCommitToKB( com.conceptualgraph.charger.obj.Graph g ) {
    }
    ...
    public void performActionRelationMatching() {
    	com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics matches = matchMasterToOpenGraphs( ef );

        String detail = "";

        // display detail for the master once
        com.conceptualgraph.kb.matching.BinaryRelationMatch match = new BinaryRelationMatch( ef.TheGraph, ef.getGraphName() );
     ...
     */
    public com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics matchMasterToOpenGraphs(EditFrame masterForMatch ) {
    	com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics matches =
                new com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics( "Matches based on " + masterForMatch.getGraphName() + " as master." );
...
//        while ( graphiter.hasNext() ) {
        for ( EditFrame nextef : Global.editFrameList.values() ) {
...
//            EditFrame nextef = graphiter.next();

            if ( nextef != masterForMatch ) {
            	com.conceptualgraph.charger.obj.Graph g = nextef.TheGraph;
    ...
    public void performActionBestRelationMatching() {
        ...
        com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics bestMatch = null;
        ...
        while ( graphiter.hasNext() ) {
            ...
            com.conceptualgraph.kb.matching.BinaryRelationMatchGroupMetrics matches = matchMasterToOpenGraphs( currentef );
