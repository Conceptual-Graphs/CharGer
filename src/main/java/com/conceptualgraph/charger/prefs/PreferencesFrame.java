 ...
 */
package com.conceptualgraph.charger.prefs;
...
public class PreferencesFrame extends javax.swing.JFrame {
    ...
    public PreferencesWriter writer = new PreferencesWriter(appearance, compat, actors, configPanel );
    ...
    public PreferencesFrame() {
        initComponents();
        prefFileName.setText( Global.getPrefFile().getAbsolutePath());
...
//        enableCraftPanel( Global.craftEnabled );
            // TODO: Give any plugin the capability to add its own prefs, not just CRAFT
//        enableCraftPanel( new ArrayList<String>( Arrays.asList( Global.moduleNamesToEnableCommaSeparated.split( ",")))
//                .contains( "CraftModulePlugin"));
        for ( ModulePlugin module : Global.modulePluginsActivated) {
            JPanel prefPanel = module.getPropertiesPanel();
            if ( prefPanel != null ) {
                mainPane.addTab( module.getDisplayName(), prefPanel );
            }
        }
        if ( Global.infoOn )
            mainPane.addTab("Config", configPanel );
    }
...
//    }
    ...
    public Action chooseWorkingDirectoryAction = new AbstractAction() {
        public Object getValue( String s ) {
            if ( s.equals( Action.NAME ) ) {
                return com.conceptualgraph.charger.Global.strs( "ChooseWorkingDirectoryLabel" );
            }
    ...
    private void performQueryForWorkingDirectory() {
        ...
        if ( newF == null ) {
            ...
            Global.GraphFolderString = newF.getAbsolutePath();
//            Global.GraphFolderFile = new File( Global.GraphFolderString );
            Global.setGraphFolder(Global.GraphFolderString, true );
            compat.GraphFolderField.setText(Global.GraphFolderString );

            Global.DatabaseFolderString = newF.getAbsolutePath();
            Global.DatabaseFolderFile = new File( Global.GraphFolderString );
            compat.wordnetDictField.setText(Global.GraphFolderString );

//            if ( Global.craftEnabled ) {
//                Global.craftModule.setGridFolderFile( newF );
//                craftPanel.gridFolderField.setText(Global.GraphFolderString );
//            }
        }
        this.validate();
        repaint();
...
//    }
    ...
    private void initComponents() {
        ...
        savePreferencesButton.setFont(new java.awt.Font("Lucida Grande", 0, 14)); // NOI18N
        ...
        prefFileName.setBackground(new java.awt.Color(255, 255, 255));
    ...
    private void savePreferencesButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_savePreferencesButtonActionPerformed
        try {
...
//            String prefsFilename = Global.userPreferencesFilename;
    ...
    private void setAllFoldersActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_setAllFoldersActionPerformed
        File newF = Global.queryForFolder( this, Global.GraphFolderFile,
                "Choose any file to use its parent directory for graph, grids and database (txt) files" );
        if ( newF == null ) {
            return;
...
//            Global.GraphFolderFile = new File( Global.GraphFolderString );
            ...
            actors.DatabaseFolderField.setText(Global.DatabaseFolderString );
