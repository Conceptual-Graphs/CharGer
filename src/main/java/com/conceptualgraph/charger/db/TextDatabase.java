package com.conceptualgraph.charger.db;
...
import com.conceptualgraph.charger.obj.Arrow;
 ...
 */
/**
 * Embodies accessibility to a tab-delimited text database. The database is
 * first "activated" -- i.e., the file is located and verified, its name stored
 * in the ActiveDatabases table in the Hub. There is a separate TextDatabase
 * instance associated with each real database. Once "activated" a database file
 * is associated with the same TextDatabase instance for the duration of the
 * session, no matter how many graphs refer to it, or how many concepts in a
 * single graph refer to it.
 *
 * @author Harry S. Delugach ( delugach@uah.edu ) Copyright 1998-2020
 * by Harry S. Delugach
 */
public class TextDatabase implements CGDatabase {
    ...
    /**
     * Initializes the file "in" for reading. Only called once per file per
     * session. Sets up access to the database file, loads up the valid names
     * from 1st line of file.
     *
     ...
     * read. Closes the actual file and re-opens it (since we can't be certain
     * the file supports a reset). Checks for any errors in case the file has
     * been modified or moved since it was last opened.
     */
    public void resetDB() {
        try {
            in.close();
    ...
    /**
     * @return list of strings in this file which are valid field or column
     * names
     */
    ...
    /**
     * @return returns the next line of the database, from wherever we last read
     * a line. null if at the end of the file Check end of file here; it doesn't
     * work!!!
     */
    public String[] getFields() throws CGFileException {
        ...
        try {
            ...
            theLine = in.readLine();
            if ( theLine == null ) {
                return null;	// we're at end of file
            }
            String field = null;
    ...
    public int getFieldPosition( String Field ) {
        ...
        for ( int FieldNum = 0; FieldNum < DBValidNames.length; FieldNum++ ) {
            if ( DBValidNames[FieldNum].equalsIgnoreCase( Field ) ) {
                return FieldNum;
            }
     ...
     * @param targettype The field name (column) whose value is being sought;
     * e.g., "Phone Number"
     * @return targetvalue column's value that matches the keytype's keyvalue;
     * e.g., "(804) 555-1212"
     */
    public String doLookup( String keytype, String keyvalue, String targettype ) {
        ...
        int max;
        if ( targetColumnNumber > keyColumnNumber ) {
            max = targetColumnNumber;
        } else {
            max = keyColumnNumber;
        }
     ...
     * this database.
     *
     * @param key index into the DBValidNames array; <code>-1</code> if there is
     * no key.
    ...
    public EditFrame setupGraph( int key ) throws CGException {
        ...
        dbconcept.setReferent(General.getSimpleFilename( currentfilename ), false );
