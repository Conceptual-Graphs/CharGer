...
import com.conceptualgraph.chargerlib.CharacterSets;
...
import com.conceptualgraph.kb.hierarchy.TypeHierarchyNode;
...
//        return Util.make2DDouble( displayRect );
        return getDisplayRect(); //Util.make2DDouble( displayRect );
    }
...
//            Global.info( "Concept \"" + this.getTypeLabel() + "\" added = " + b + " " + com.conceptualgraph.kb.showConceptTypeHierarchy() );
        if ( node == null ) {
            return false;
        } else {
...
//            Global.info( "Concept \"" + this.getTypeLabel() + "\" removed = " + b + " " + com.conceptualgraph.kb.showConceptTypeHierarchy() );
        return b;
     ...
     *
     * @param com.conceptualgraph.kb Notio knowledge base, to be used for storing the concept type.
     */
    /* REMOVE-NOTIO public void updateForNotio(  notio.KnowledgeBase com.conceptualgraph.kb )
     {
     try {
     notio.ConceptType t = null;
     if ( textLabel.equalsIgnoreCase( "T" ) )
     t = com.conceptualgraph.kb.getConceptTypeHierarchy().getTypeByLabel( notio.ConceptTypeHierarchy.UNIVERSAL_TYPE_LABEL );
     else if ( textLabel.equalsIgnoreCase( "null" ) )
     t = com.conceptualgraph.kb.getConceptTypeHierarchy().getTypeByLabel( notio.ConceptTypeHierarchy.ABSURD_TYPE_LABEL );
     else
     t = com.conceptualgraph.kb.getConceptTypeHierarchy().getTypeByLabel( textLabel );
     // @bug doesn't check whether type's old type label should be deleted  (see R0014)
     if ( t == null )
     {
     nxConceptType = new notio.ConceptType( textLabel );
     com.conceptualgraph.kb.getConceptTypeHierarchy().addTypeToHierarchy( nxConceptType );
     }
     else
     {
