...
//                Global.info( "calcDimensions: arrow from " + fromObj.getTextLabel() + " to " + toObj.getTextLabel() );
        ...
        double handleSize = 8;
        Point2D.Double mid = General.midPoint( fromPt, toPt );

        this.setDisplayRect( mid.x -  handleSize / 2, mid.y -  handleSize / 2, handleSize, handleSize);
...
//            Global.info( "calling findclippingpointshape");
    ...
    public static Point2D.Double findClippingPointRectangle( Point2D.Double fromPt, Rectangle2D.Double rect ) {
        ...
        Rectangle2D.Double r = General.make2DDouble( rect );
...
//        Global.info( "find clipping point: fromPt " + fromPt.toString() + "; toPt " + toPt.toString() );
        ...
        Point2D.Double mid = General.midPoint( from, to );
...
//    }
    ...
    /**
     * Determines whether the two com.conceptualgraph.charger nodes are linked together, by any
     * GEdge.
     *
     * @param go1 one node
     ...
     * gn is its "to" node.
     * @see GEdge.Direction#FROM
     * @see GEdge.Direction#TO
     * @see GEdge.Direction#UNLINKED
     */
    public GEdge.Direction howLinked( GNode gn ) {
        if ( gn == fromObj ) {
            return GEdge.Direction.FROM;
        }
        if ( gn == toObj ) {
            return GEdge.Direction.TO;
        }
        return GEdge.Direction.UNLINKED;
    }
