...
import com.conceptualgraph.charger.IOManager;
import com.conceptualgraph.charger.exception.CGContextException;
import com.conceptualgraph.charger.util.CGUtil;
import com.conceptualgraph.charger.util.ObjectLocator;
import com.conceptualgraph.chargerlib.Rectangle3D;
import com.conceptualgraph.chargerlib.WrappedText;
import com.conceptualgraph.chargerlib.history.HistoryRecord;
import com.conceptualgraph.chargerlib.history.ObjectHistory;
import com.conceptualgraph.kb.KnowledgeBase;
...
//    public final static Color defaultSelectColor = Color.black;
    ...
    public static Rectangle3D defaultDim = new Rectangle3D( 0, 0,
            Float.parseFloat( Global.Prefs.getProperty( "defaultGraphObjectWidth", "40" ) ),
            Float.parseFloat( Global.Prefs.getProperty( "defaultGraphObjectHeight", "30" ) ),
            Float.parseFloat( Global.Prefs.getProperty( "defaultGraphObjectDeptht", "1" ) )
            );
...
//    public static int defaultDepth = (int)
//    );
...
//    /**
//     * The depth (3D) of a graph object's display. If 1 then it's 2D. If it's zero, then it should be invisible (not implemented)
//     */
...
//            Global.info( "graph object force move of " + getTextLabel());
        double oldx = getDisplayRect().x;
        double oldy = getDisplayRect().y;
        
        displayRect3D.x += delta.x;     // Note: directly setting part of display rectangle here.
        displayRect3D.y += delta.y;
                
        return ( (oldx != getDisplayRect().x) || (oldy != getDisplayRect().y));
    }
    ...
    public void setCenterOnly( Point2D.Double p ) {
        if ( p != null ) {
            displayRect3D.x = p.x - displayRect3D.width / 2;
            displayRect3D.y = p.y - displayRect3D.height / 2;
    ...
    public void setUpperLeft( Point2D.Double upperleft ) {
        displayRect3D.x = upperleft.x;
        displayRect3D.y = upperleft.y;
    }
    ...
    public Point2D.Double getCenter() {
        return new Point2D.Double( getUpperLeft().x + getDisplayRect().width / 2.0f, getUpperLeft().y + getDisplayRect().height / 2.0f );
    }
    ...
    public void setDim( Dimension d ) {
        setDisplayRect( getDisplayRect().x, getDisplayRect().y, (double)d.width, (double)d.height );
    }
    ...
    public void setDim( float wid, float ht ) {
        setDisplayRect( getDisplayRect().x, getDisplayRect().y, (double)wid, (double)ht );
    }

    public Rectangle3D getDisplayRect3D() {
        return displayRect3D;
    }

    public void setDisplayRect3D( Rectangle3D displayRect3D ) {
        this.displayRect3D = displayRect3D;
    }
...
//        return displayRect;
        return displayRect3D.getRectangle2D();
    }
    ...
    public void setDisplayRect( Rectangle2D.Double rect ) {
        displayRect3D.setRectangle2D( rect );
...
//        setCenterOnly( getCenter() );
    ...
    public void setDisplayRect( double x, double y, double width, double height ) {
        this.setDisplayRect( new Rectangle2D.Double( x, y, width, height ));
    }
     ...
     * @return the depth of the object display
     */
    public double getDepth() {
        return displayRect3D.getDepth();
    }
...
//        }
        ...
        if ( myKind == GraphObject.Kind.GRAPH ) {
            Graph g = ( (Graph)this );
            g.resizeForContents( p );
            g.adjustEdges();

        } else if ( myKind == GraphObject.Kind.GNODE ) // warning: uses sub-class knowledge here!!!
        {
            Dimension contentSize = getMinimumContentSize();
            double width = displayRect3D.width;
            double height = displayRect3D.height;
            boolean rectChanged = false;

            if ( displayRect3D.getWidth() < contentSize.getWidth() ) {
                rectChanged = true;
                width = contentSize.width;
            }

            if ( displayRect3D.getHeight() < contentSize.getHeight() ) {
                rectChanged = true;
                height = contentSize.height;
            }

            if ( rectChanged ) {
                displayRect3D.width = width;
                displayRect3D.height = height;
                if ( p != null ) {
                    setCenterOnly( p );
                }
            }
   ...
   public Dimension getMinimumContentSize() {
    ...
    public String toString() {
        ...
        return CGUtil.shortClassName( this ) + "|" + objectID + "," + ownerID
                + "|" + textLabel + "|" + getDisplayRect().x + "," + getDisplayRect().y
                + "," + getDisplayRect().width + "," + getDisplayRect().height
     ...
     * @see com.conceptualgraph.charger.xml.CGXParser#parseCGXMLString
     ...
     * @see com.conceptualgraph.charger.EditManager
     */
    public static String listToStringXML( ArrayList list ) {
        ...
        while ( iter.hasNext() ) {
            ...
            GraphObject go = (GraphObject)iter.next();
            if ( go instanceof Graph ) {
                s.append( com.conceptualgraph.charger.cgx.CGXGenerator.GraphObjectXML( (Graph)go, "" ) ); //+ Hub.LineSeparator );
            } else {
                s.append( com.conceptualgraph.charger.cgx.CGXGenerator.GraphObjectXML( go, "" ) ); //+ Hub.LineSeparator );
            }
        }
...
//    }
     ...
     * @see com.conceptualgraph.charger.util.ObjectLocator
     */
    public ObjectLocator getObjectLocator() {
        ...
        //com.conceptualgraph.charger.Global.info( "class used is " + this.getClass().getName() );
