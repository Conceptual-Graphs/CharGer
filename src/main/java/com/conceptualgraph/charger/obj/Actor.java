   ...
   public void setCenter( Point2D.Double p) {
	...
	public Shape getShape()
	{
		//Rectangle2D.Double r = Util.make2DDouble( displayRect );
            Rectangle2D.Double r = this.getDisplayRect();
...
//		g.drawString( textLabel, textLabelLowerLeftPt.x, textLabelLowerLeftPt.y );
   ...
   {
                Global.info( "actor custom display rect - before is " + displayRect3D.toString() );
        double xinc = getDisplayRect().width * horizontalStretch;
        double yinc = getDisplayRect().height * verticalStretch;
        double x = getDisplayRect().x - xinc / 2f;
        double y = getDisplayRect().y - yinc / 2f;
        double width = getDisplayRect().width + xinc;
        double height = getDisplayRect().height + yinc;
...
//        displayRect = new Rectangle2D.Double( x, y, width, height );
        ...
        setCenter();        // force edges to be redrawn
                Global.info( "actor custom display rect - after is " + displayRect3D.toString() );
   }
...
*/
	...
	/**
		...
		@param com.conceptualgraph.kb Notio knowledge base, to be used for consistency checking.
	*/
	/* REMOVE-NOTIO public void updateForNotio( notio.KnowledgeBase com.conceptualgraph.kb )
	{
		...
		nxRelationType = com.conceptualgraph.kb.getRelationTypeHierarchy().getTypeByLabel( textLabel );
		if ( nxRelationType == null )
		{
			nxRelationType = new notio.RelationType( textLabel );
			com.conceptualgraph.kb.getRelationTypeHierarchy().addTypeToHierarchy( nxRelationType );
		}
