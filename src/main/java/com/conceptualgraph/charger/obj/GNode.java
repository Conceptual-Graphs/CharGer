...
import com.conceptualgraph.charger.*;
import com.conceptualgraph.charger.act.*;
...
abstract public class GNode extends GraphObject  {
     ...
     * TypeDescriptor
     *
     * @see com.conceptualgraph.charger.obj.AbstractTypeDescriptor
     */
...
//        displayRect.setFrame( displayRect.x, displayRect.y, (float)GraphObject.defaultDim.width, (float)GraphObject.defaultDim.height );
        setDisplayRect( getDisplayRect().x, getDisplayRect().y, (double)GraphObject.defaultDim.width, (double)GraphObject.defaultDim.height );
    }
     ...
     * @see com.conceptualgraph.charger.act.GMark#setChanged
     */
     ...
     * arrows directed toward this node, or <code>GEdge.Direction.TO</code> (for
    ...
    public ArrayList<GNode> getLinkedNodes( GEdge.Direction direction ) {
        ArrayList<GNode> holder = new ArrayList<>();
     ...
     * Equivalent to the union of the lists returned
     * by getLinkedNodes( from ) and getLinkedNodes( to )
     * @return the list of nodes that are connected in either direction
     */
    public ArrayList<GNode> getLinkedNodes() {
        ArrayList<GNode> combinedList = new ArrayList<>();
        
        ArrayList fromList = getLinkedNodes( GEdge.Direction.FROM);
        ArrayList toList = getLinkedNodes( GEdge.Direction.TO);
     ...
     *
     * @param com.conceptualgraph.kb Notio knowledge base, to be used for consistency checking. HSD
     * this should be replaced by a new call to the com.conceptualgraph.charger com.conceptualgraph.kb
     */
    //REMOVE-NOTIO abstract void updateForNotio(  notio.KnowledgeBase com.conceptualgraph.kb );
    /**
    ...
    public void adjustEdges() {
        //com.conceptualgraph.charger.Global.info( "adjusting " + myedges.size() + " edges for node " + getTextLabel() );
        int k;
        for ( k = 0; k < myedges.size(); k++ ) {
            GEdge edge = (GEdge)myedges.get( k );
            edge.placeEdge();
            if ( edge.fromObj.ownerGraph == edge.toObj.ownerGraph )
                edge.setOwnerGraph( edge.fromObj.ownerGraph);
        }
    }
...
//                    (float)getUpperLeft().x, (float)getUpperLeft().y - 2 );
     ...
     * 
     * @param label The type label to set
     * @param resize Shether to resize if necessary.
     */
...
//    public boolean commitToKnowledgeBase( com.conceptualgraph.kb.KnowledgeBase com.conceptualgraph.kb ) {
//        return true;
//    }
//
//    public boolean unCommitFromKnowledgeBase( com.conceptualgraph.kb.KnowledgeBase com.conceptualgraph.kb ) {
//        return true;
//    }
}
