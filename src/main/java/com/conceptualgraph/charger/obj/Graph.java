...
import com.conceptualgraph.charger.EditManager;
import com.conceptualgraph.charger.EditingChangeState;
import com.conceptualgraph.charger.Global;
import com.conceptualgraph.charger.exception.CGContextException;
import com.conceptualgraph.charger.gloss.AbstractTypeDescriptor;
import com.conceptualgraph.charger.layout.SpringGraphLayout;
...
import com.conceptualgraph.chargerlib.WrappedText;
import com.conceptualgraph.chargerlib.history.ObjectHistoryRecord;
import com.conceptualgraph.kb.KBException;
 ...
 * Harry S. Delugach.
 */
public class Graph extends Concept implements Printable, com.conceptualgraph.kb.KnowledgeSource { // GNode {
...
//    public String createdTimeStamp = General.getFormattedCurrentDateTime();
//    public String modifiedTimeStamp = General.getFormattedCurrentDateTime();
...
//        }
    ...
    public Shape getShapeForDrawing() {
        Rectangle2D.Double rect = getDisplayRect();  //Util.make2DDouble( displayRect );
    ...
    public Rectangle2D.Double getRegionAvailableForContent() {
        ...
        x = getDisplayRect().x + contextBorderWidth + contextInnerPadding;
        y = getDisplayRect().y + contextBorderWidth + contextInnerPadding + labelHeight;
        double textWidthNeeded = textsize.width;
        width = getDisplayRect().width - 2 * contextBorderWidth - 2 * contextInnerPadding;
        if ( textWidthNeeded < width ) {
            width = textWidthNeeded - 2 * contextBorderWidth - 2 * contextInnerPadding;
        }
        height = getDisplayRect().height - 2 * contextBorderWidth - 2 * contextInnerPadding - getTextLabelSize().height;
    ...
    public void setRegionAvailableForContent( Rectangle2D.Double rect ) {
        ...
        setDisplayRect(
                rect.x - contextBorderWidth - contextInnerPadding,
                rect.y - contextBorderWidth - contextInnerPadding - labelHeight,
                rect.width + 2 * contextBorderWidth + 2 * contextInnerPadding,
                rect.height + 2 * contextBorderWidth + 2 * contextInnerPadding + getTextLabelSize().height
        );
    }


    /**
     * Draws the border of a context. com.conceptualgraph.Graphs are handled specially because to
...
//            g2D.setFont( oldFont );
        ...
        Iterator<GraphObject> iter = new ShallowIterator( this, GraphObject.Kind.GRAPH );
        GraphObject go;
        while ( iter.hasNext() ) {
            go = iter.next();
        ...
        if ( !printing ) {
            if ( isSelected && this.myKind == GraphObject.Kind.GRAPH && this.getOwnerGraph() != null ) {
                ...
                Rectangle2D.Double drect = getDisplayRect(); // (Rectangle2D.Double)displayRect.clone();
                g2D.draw( drect );
    ...
    /*============ delete operations
     com.conceptualgraph.charger	notio	op
     yes		no		delete arrow
...
//        GraphObject igo = null;
        ...
        for ( GraphObject obj : objectsToMove ) {
            GraphObject go = obj;
            if ( go instanceof Graph ) {
                graphs.add( (Graph)go );
...
//                Global.info( "Moved non-graph node " + node.getTextLabel() + " by " + delta );
    ...
    public void insertObject( GraphObject go ) {
        ...
        if ( go instanceof GEdge && ( (GEdge)go ).fromObj != null && ( (GEdge)go ).toObj != null /*   && ( (GEdge)go ).toObj.getOwnerGraph() != ( (GEdge)go ).fromObj.getOwnerGraph() */ ) {
            ...
            ArrayList endGos = new ArrayList();
            endGos.add( ( (GEdge)go ).toObj );
            endGos.add( ( (GEdge)go ).fromObj );
            try {
                Graph dominant = GraphObject.findDominantContext( endGos );
...
//
     ...
     * false
     */
    public boolean containsGraphObjects( ArrayList<GraphObject> graphobjects ) {
        Iterator<GraphObject> iter = new ShallowIterator( this, GraphObject.Kind.GNODE );
        GraphObject go = null;
        while ( iter.hasNext() ) {
            go = iter.next();
    ...
    public void handleContextLinks() {
        ...
        Iterator<GraphObject> graphs = new ShallowIterator( this, GraphObject.Kind.GRAPH );
        GraphObject go = null;
        while ( graphs.hasNext() ) {
            ( (Graph)graphs.next() ).handleContextLinks();
        }

        Iterator<GraphObject> links = new ShallowIterator( this, GraphObject.Kind.GEDGE );
        GEdge ge = null;
        while ( links.hasNext() ) {
            ge = (GEdge)links.next();
...
//                            Global.info( "moving GEdge " + ge.objectID + " to graph " + newOwner.objectID );
    ...
    public void abandonObject() {
        Iterator<GraphObject> iter = new DeepIterator( this );
        while ( iter.hasNext() ) {
            GraphObject go = iter.next();
            //this.eraseObject( go );
            //try {
    ...
    protected void finalize() throws Throwable {
        try {
            ...
            Iterator<GraphObject> iter = new ShallowIterator( this );
            while ( iter.hasNext() ) {
                GraphObject go = iter.next();
    ...
    public Rectangle2D.Double getContentBounds() {
        ...
        while ( iter.hasNext() ) {
            ...
            if ( go.myKind != GraphObject.Kind.GEDGE ) {
                Rectangle2D.Double rect = go.getDisplayRect();
//                Global.info( "adding bounds for object " + go.getTextLabel() + " rect is " + go.getDisplayRect() );
                if ( biggest == null ) {
                    biggest = General.make2DDouble( go.getDisplayRect() );
                } else {
...
//                Global.info( "biggest rect is now " + biggest );
        ...
        double textWidthNeeded = textsize.width;
        double width = getDisplayRect().width - 2 * contextBorderWidth - 2 * contextInnerPadding;
        if ( textWidthNeeded < width ) {
            width = textWidthNeeded - 2 * contextBorderWidth - 2 * contextInnerPadding;
        }
...
//        this.adjustDisplayRect( null );
    ...
    public String toStringDeep( boolean deep ) { //, ArrayList deferredNodes ) {
        ...
        if ( deep ) {
            Object dummy;
            Stack<GraphObject> inOrder = new Stack();		// because enumerate returns items in reverse order...
            while ( iter.hasNext() ) {
                GraphObject go = iter.next();
     ...
     */
   /* public void updateForNotio( notio.KnowledgeBase com.conceptualgraph.kb ) {
        NotioCharGerLookup.clear();

        super.updateForNotio( com.conceptualgraph.kb );
    ...
    * */
    ...
    public ArrayList<GraphObject> getGraphObjects() {
        ArrayList<GraphObject> list = new ArrayList<>( 10 );
        Iterator<GraphObject> iter = this.graphObjects();
        while ( iter.hasNext() ) {
            list.add( iter.next() );
        }
        return list;
     ...
     * @param edgeLength the equilibrium edge length desired.
     * @see SpringGraphLayout
     */
    
    public void layoutGraph( double edgeLength ) {
        if ( objectHashStore.isEmpty() ) return;
        SpringGraphLayout layout = new SpringGraphLayout( this );
        layout.setEquilibriumEdgeLength( edgeLength );
     ...
     * @param he
     */
    public void addHistory( ObjectHistoryRecord he ) {
        DeepIterator iter = new DeepIterator( this );
        while ( iter.hasNext() ) {
            iter.next().addHistoryRecord( he );
        }
    }
    ...
    public AbstractTypeDescriptor[] findTypeDescriptors( String term ) {
        ...
        while ( iter.hasNext() ) {
            ...
            for ( int k = 0; k < ds.length; k++ ) {
                Global.info( "found descriptors! checking term " + ds[ k].getLabel() + " and " + term );
     ...
     * @param kb 
     */
    public boolean commitToKnowledgeBase( com.conceptualgraph.kb.KnowledgeBase kb ) {
        return true;
        //boolean b = com.conceptualgraph.kb.getConceptTypeHierarchy().addLabel( this.getTypeLabel() );
        //    Hub.consoleMsg( "Concept \"" + this.getTypeLabel() + "\" added = " + b + " " + com.conceptualgraph.kb.showConceptTypeHierarchy() );
    }
     ...
     * @param kb 
     */
    public boolean unCommitFromKnowledgeBase( com.conceptualgraph.kb.KnowledgeBase kb ) {
        return true;
        //boolean b = com.conceptualgraph.kb.getConceptTypeHierarchy().addLabel( this.getTypeLabel() );
        //    Hub.consoleMsg( "Concept \"" + this.getTypeLabel() + "\" added = " + b + " " + com.conceptualgraph.kb.showConceptTypeHierarchy() );
    }
