...
public class WNUtil {
    ...
    public static int wordCount( String s, boolean smart ) {
        if ( smart ) {
            ...
            StringTokenizer t = new StringTokenizer( s, com.conceptualgraph.charger.gloss.wn.WNUtil.delimiters );
    ...
    public static JTable getDescriptorTable( AbstractTypeDescriptor[] descriptors ) {
        ...
        TableModel readonlymodel
				...
				//Craft.say( "table created with " + dvector.size() + " rows" );
        //JTable tab = new JTable( readonlymodel );
        //com.conceptualgraph.charger.util.Util.adjustTableColumnWidths( tab );
        com.conceptualgraph.chargerlib.TableSorter sorter = new com.conceptualgraph.chargerlib.TableSorter( readonlymodel );
        JTable tab = new JTable( sorter );
        sorter.addMouseListenerToHeaderInTable( tab );

        General.setTableColumnWidth( tab.getColumn( "Type" ), 60 );
        General.setTableColumnWidth( tab.getColumn( "Term" ), 120 );
        General.setTableColumnWidth( tab.getColumn( "POS" ), 60 );
        General.setTableColumnWidth( tab.getColumn( "Seq" ), 60 );
    ...
    private static Vector getDescriptorVector( int sequence, AbstractTypeDescriptor d ) {
        ...
        //Craft.say( "get descriptor vector, v size is " + v.size() );
        return v;
    }
    ...
    public static void queryForDictFolder( JFrame f ) {
        File newF = com.conceptualgraph.charger.Global.queryForFolder(f, new File( com.conceptualgraph.charger.gloss.wn.WordnetManager.wordnetDictionaryFilename ),
                "Choose any file to use its parent directory as Wordnet's dictionary file" );
        if ( newF == null ) {
            return;
        } else {
        	com.conceptualgraph.charger.gloss.wn.WordnetManager.wordnetDictionaryFilename = newF.getAbsolutePath();
        }
    }
