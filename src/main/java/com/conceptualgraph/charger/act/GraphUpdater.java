package com.conceptualgraph.charger.act;
...
public class GraphUpdater implements Runnable {
    ...
    /**
     * key is specific com.conceptualgraph.charger.obj.Actor, entry is the plugin instance
    ...
    public static void startupAllActors( Graph g ) {
        ...
        Iterator<GraphObject> actors = new DeepIterator( g, new Actor() );
     ...
     * start with "com.conceptualgraph.actorplugin" and end with ".class" that do NOT have a '$' in them.
     * Returns a list of names without the leading "com.conceptualgraph.actorplugin/" in case file
...
//            Global.consoleMsg( "looking for plugins in " + classPathSegment[cpsIndex] );
...
//                    Global.info( "jar entry name is " + j.getName() );
                    ...
                    if ( name.startsWith( "actorplugin" ) && name.endsWith( ".class" )
                            && ( !name.contains( "$" ) ) ) {
                        classfiles.add( name );
                    }
                }
                ...
                pluginDirectoryFile = new File( classPathSegment[cpsIndex]
                        + File.separator + "actorplugin" );

                if ( pluginDirectoryFile.exists() ) {

                 Global.consoleMsg( "plugin directory is " + pluginDirectoryFile.getAbsolutePath() );
...
//        String classpath = System.getProperty( "java.class.path" );
        ...
        for ( int n = 0; n < plugins.length; n++ ) {
            ...
            String className = General.stripFileExtension( plugins[ n] );
            //Global.info( "plugin number " + n + " is " + className );
                String name = className;
            try {
                // in a jar file, still has prefix, but not when classes are in separate files
                if ( !className.startsWith( "actorplugin" ) ) {
                    name = "actorplugin" + separator + name;
                } else {
...
//                		Global.info( "found plugin " + className );
                ...
                Global.info( "Plugin " + name + " not found exception: " + cnfe.getMessage() );
            }
    ...
    public synchronized void updateGraph( Graph g ) {
        ...
        Iterator<GraphObject> nodes = new ShallowIterator( g, GraphObject.Kind.GNODE );
        GNode gn = null;
        while ( nodes.hasNext() ) {
            gn = (GNode)nodes.next();
            try {
...
//        Global.info( "updating coref from " + gn.toString() );
    ...
    public synchronized void fireActor( Actor a, ArrayList inputs, ArrayList outputs )
        ...
        if ( hasAttribute( a, "primitive" ) && isExecutable( a ) ) {
            try {
                possibleChange = true;
    ...
    public static boolean isExecutable( Actor a ) {
        return hasAttribute( a, (Object)"executable" );
    }

    /**
     * Convenience method for actor builders. Constructs a list of
     * com.conceptualgraph.charger.obj.Concept suitable for use in an actor signature. If differing
     ...
     * @return ArrayList of com.conceptualgraph.charger.obj.Concept instances, all with the same
     * type label
     */
    public static ArrayList<Concept> createConceptList( int numberOfConcepts, String typeLabel ) {
        ArrayList<Concept> cons = new ArrayList<>();
        for ( int n = 0; n < numberOfConcepts; n++ ) {
            AddStringAsConceptType( cons, typeLabel );
        }
        return cons;
    }
