...
@see com.conceptualgraph.actorplugin.QuantityBarPlugin
 */
public class StockQuotePlugin implements com.conceptualgraph.charger.act.ActorPlugin
{
	...
	private com.conceptualgraph.charger.EditFrame owner = null;
	/**
		...
		throws com.conceptualgraph.charger.exception.CGActorException
	{
		...
		if ( inputs.size() > 0 )
		{
			...
			try {
				owner = ((com.conceptualgraph.charger.obj.Concept)inputs.get( 0 )).getOutermostGraph().getOwnerFrame();
			} catch ( NullPointerException e ) { owner = null; }
		}
		if ( com.conceptualgraph.charger.Global.traceActors )
			com.conceptualgraph.charger.Global.info( "inref is " + inref );
	...
	private float getQuote( String tickerSymbol )
	{
		try 
		{
			...
			{
						//com.conceptualgraph.charger.Global.info( "input line is " + inputLine );
				...
				if ( inputLine.indexOf( pattern ) != -1 )
				{
					...
					com.conceptualgraph.charger.Global.info( "I think that " + result + " is the result." );
					return result;
				}
			}
			in.close();
		} catch ( MalformedURLException me ) { com.conceptualgraph.charger.Global.warning( "Bad URL: " + me.getMessage() );
		} catch ( IOException ie ) 
		{ 
			if ( ! alreadyWarned )
			{
				alreadyWarned = true;
				com.conceptualgraph.charger.Global.error( "IO Exception " + ie.getMessage() );
