...
/**
	Performs a basic clock "pulse" function for com.conceptualgraph.charger. Actor's name is "pulse".
 */
public class PulsePlugin implements com.conceptualgraph.charger.act.ActorPlugin
{
...
//                    = DateFormat.getTimeInstance( DateFormat.MEDIUM ).format( Calendar.getInstance().getTime() );
        ( (Concept)gnodeToUpdate ).setReferent( ( (Concept)gnodeToUpdate ).getReferent(), true );
				...
				throws com.conceptualgraph.charger.exception.CGActorException
	{
		...
		if ( inputs.size() == 2 )
		{
			intype1 = (String)((Concept)(inputs.get( 0 ))).getTypeLabel();
			inref1 = (String)((Concept)(inputs.get( 0 ))).getReferent();

			inref2 = (String)((Concept)(inputs.get( 1 ))).getReferent();
			intype2 = (String)((Concept)(inputs.get( 1 ))).getTypeLabel();
		}
		...
		try
		{
			...
			Global.info( "Displaying the value for pulse plugin " + interval );
		if ( enableType.equalsIgnoreCase( "T") )
		{
					// set up the output node for updating
		//if ( outputs.size() > 0 )
			com.conceptualgraph.charger.EditFrame ownerFrame =
				((Concept)(inputs.get( 0 ))).getOutermostGraph().getOwnerFrame();
			gnodeToUpdate = (GNode)(outputs.get( 0 ));
