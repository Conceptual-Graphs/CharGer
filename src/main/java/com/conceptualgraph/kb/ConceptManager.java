//
//  com.conceptualgraph.kb.ConceptManager.java
//  CharGer 2003
//
//  Created by Harry Delugach on Sun May 11 2003.
//
package com.conceptualgraph.kb;

import com.conceptualgraph.charger.EditFrame;
import com.conceptualgraph.charger.Global;
import com.conceptualgraph.charger.EditingChangeState.EditChange;
import com.conceptualgraph.charger.gloss.AbstractTypeDescriptor;
import com.conceptualgraph.charger.gloss.wn.WordnetTypeDescriptor;
import com.conceptualgraph.charger.obj.Concept;
import com.conceptualgraph.charger.obj.DeepIterator;
import com.conceptualgraph.charger.obj.GEdge;
import com.conceptualgraph.charger.obj.GNode;
import com.conceptualgraph.charger.obj.GenSpecLink;
import com.conceptualgraph.charger.obj.GraphObject;
import com.conceptualgraph.charger.obj.Relation;
import com.conceptualgraph.charger.obj.TypeLabel;
import com.conceptualgraph.craft.Craft;
import com.conceptualgraph.craft.Reporter;
...
public class ConceptManager {
    ...
    public static JTable getBinaryTupleTable( ConceptsToInclude selector ) {
        ...
        if ( Craft.useOnlyBinaryRelationsinCraft ) // TODO - change get..Tuples and getTupleColumnLabels back to Vector because that's what's needed.
        {
            readonlymodel = new DefaultTableModel( tableValues, com.conceptualgraph.kb.BinaryTuple.getTupleColumnLabels().toArray() ) {
...
//            readonlymodel = new DefaultTableModel( getAllRelationTuples( selector ), com.conceptualgraph.kb.BinaryTuple.getTupleColumnLabels().toArray() ) {
//                public boolean isCellEditable( int rowIndex, int columnIndex ) {
//                    return false;
//                }
//            };
...
//            readonlymodel = new DefaultTableModel( binaryRelationTuples, tupleColumnLabels ) {
    ...
    public static ArrayList getAll( GraphObject pattern ) {
        ...
        while ( graphiter.hasNext() ) {
            ...
            com.conceptualgraph.charger.obj.Graph g = ef.TheGraph;
            //Craft.say( "graph g is " + g + "\n" + g.getBriefSummary() );
            Iterator<GraphObject> iter = new DeepIterator( g, pattern );
            while ( iter.hasNext() ) {
    ...
    public static Vector getAllConcepts( ConceptsToInclude selector ) {
        ...
        //Craft.say( "graph list has " + Hub.graphList.size() + " elements." );
        Iterator graphiter = Global.editFrameList.values().iterator();
        ...
        while ( graphiter.hasNext() ) {
            ...
            com.conceptualgraph.charger.obj.Graph g = ef.TheGraph;
            //Craft.say( "graph g is " + g + "\n" + g.getBriefSummary() );
            Iterator<GraphObject> iter = new DeepIterator( g, GraphObject.Kind.CONCEPT_OR_GRAPH );
            while ( iter.hasNext() ) {
                ...
                if ( selector == ConceptsToInclude.ALL || ( selector == ConceptsToInclude.GENERIC_ONLY && c.getReferent().equals( "" ) ) ) {
                    ...
                    //Craft.say( "v " + k + " has label " + v.get( k ) );
                }
            }
        }
     ...
     *
     * @param g The com.conceptualgraph.charger graph whose relations are to be examined.
     * @param selector Which concepts to gather.
     * @return a vector of Table Entry vectors
     * @see #makeTableEntryArrayList
     */
    public static ArrayList<BinaryTuple> getBinaryRelationTuples(com.conceptualgraph.charger.obj.Graph g, com.conceptualgraph.kb.ConceptManager.ConceptsToInclude selector ) {
    ...
    public static ArrayList<BinaryTuple> getAllRelationTuples( ConceptsToInclude selector ) {
        ...
        com.conceptualgraph.charger.obj.Graph[] allGraphs = Global.knowledgeManager.getAllGraphs();
        // for every available graph
        for ( int gnum = 0; gnum < allGraphs.length; gnum++ ) {
        	com.conceptualgraph.charger.obj.Graph g = allGraphs[ gnum];
            {
                // for every relation node in the graph
                Iterator<GraphObject> nodeiter = new DeepIterator( g, GraphObject.Kind.GNODE );
                while ( nodeiter.hasNext() ) {
                    GNode gn = (GNode)nodeiter.next();
                    if ( gn instanceof com.conceptualgraph.charger.obj.Relation || gn instanceof com.conceptualgraph.charger.obj.Actor ) {
                        //Craft.say( " found a relation/actor " + gn.getTextLabel() );
                        GNode rel = gn;
...
//                                ArrayList v = makeTableEntryArrayList( c1, rel, c2, selector );
    ...
    /**
     * Constructs a single table entry from a com.conceptualgraph.charger concept-relation-concept
     * set.
     *
    ...
    protected static ArrayList makeTableEntryArrayList(
            Concept c1, com.conceptualgraph.charger.obj.GNode rel, Concept c2, ConceptsToInclude selector ) {
        ...
        if ( selector == ConceptsToInclude.ALL
            ...
            //Craft.say( "concept " + c1.getTextLabel() + " included." );
            ...
            //		Craft.say( "v " + i + " has label " + v.get( i ) );
            return v;
        } else {
            return null;
        }
    ...
    public static String convertTriplesToHTMLtable( ArrayList triples, String summary ) {
        String XMLSummary = com.conceptualgraph.chargerlib.XMLGenerator.quoteForXML( summary.trim() );
    ...
    public static String binaryRelationTableEntryToHTML( ArrayList tableEntry ) {
        ...
        col = com.conceptualgraph.kb.ConceptManager.COL_CONCEPT_LABEL_1;
        html += " <TD>"
                + com.conceptualgraph.chargerlib.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";

        col = com.conceptualgraph.kb.ConceptManager.COL_RELATION_LABEL;
        html += " <TD>"
                + com.conceptualgraph.chargerlib.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";

        col = com.conceptualgraph.kb.ConceptManager.COL_CONCEPT_LABEL_2;
        html += " <TD>"
                + com.conceptualgraph.chargerlib.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";
        return html;
...
//
     ...
     */
    private static boolean swapBinaryRelation(Concept c1, com.conceptualgraph.charger.obj.GNode rel, Concept c2 ) {
...
//                );
//    }
    public static String makeSentence(Concept c1, com.conceptualgraph.charger.obj.GNode rel, Concept c2 ) {
        if ( rel instanceof com.conceptualgraph.charger.obj.Relation ) {
            return Reporter.natLang( (com.conceptualgraph.charger.obj.Relation)rel, false );
        } else {
            return makeSentenceOLD( c1, rel, c2 );
        }
    }

    public static String makeSentenceOLD(Concept c1, com.conceptualgraph.charger.obj.GNode rel, Concept c2 ) {
        String c1label = null;
        String c2label = null;
        String rlabel = null;
         ...
         */
        if ( swap ) {
        	com.conceptualgraph.charger.obj.Concept temp = c1;
            c1 = c2;
            c2 = temp;
        }
        ...
        conn2 = Reporter.tmap.suffix( rlabel, rel instanceof com.conceptualgraph.charger.obj.Actor );
        conn1 = Reporter.tmap.prefix( rlabel, rel instanceof com.conceptualgraph.charger.obj.Actor );
     ...
     * turns up.
     */
    public static com.conceptualgraph.charger.gloss.AbstractTypeDescriptor[] getSensesFromPhrase(String phrase, com.conceptualgraph.charger.gloss.wn.WordnetManager wnmgr ) {
        ArrayList descriptors = new ArrayList();
        StringTokenizer toks = null;

        toks = new StringTokenizer( phrase, com.conceptualgraph.charger.gloss.wn.WNUtil.delimiters );
        // prepare a vector with all the tokens in it
        ArrayList words = new ArrayList();
        while ( toks.hasMoreTokens() ) {
            String[] ws = com.conceptualgraph.charger.gloss.wn.WNUtil.guessWordsFromPhrase( toks.nextToken() );
        ...
        while ( wordnumber < words.size() && keepGoing ) {
            ...
            com.conceptualgraph.charger.gloss.AbstractTypeDescriptor descr = findLongestTermAlreadyDefined( words, wordnumber );
            if ( descr != null ) {
                ...
                {
                    StringTokenizer temptoks = new StringTokenizer( alreadyDefinedLabel, com.conceptualgraph.charger.gloss.wn.WNUtil.delimiters );
        ...
        return (com.conceptualgraph.charger.gloss.AbstractTypeDescriptor[])descriptors.toArray(new com.conceptualgraph.charger.gloss.AbstractTypeDescriptor[ 0 ] );
    }
     ...
     */
    public static com.conceptualgraph.charger.gloss.AbstractTypeDescriptor findLongestTermAlreadyDefined(ArrayList wordlist, int startWord ) {
        ...
        for ( int wordnum = wordlist.size() - 1; wordnum >= startWord; wordnum-- ) {
            ...
            com.conceptualgraph.charger.gloss.AbstractTypeDescriptor d[] = Global.knowledgeManager.findTypeDescriptor( term, false );
     ...
     */
    public static com.conceptualgraph.charger.obj.Graph makeTypeHierarchy() {
    	com.conceptualgraph.charger.obj.Graph h = new com.conceptualgraph.charger.obj.Graph();
    	com.conceptualgraph.charger.obj.Graph[] hierarchies = Global.knowledgeManager.findGraphsWithTypeHierarchy();
     ...
     */
    private static void fillInGlossaryDefinitions( com.conceptualgraph.charger.obj.Graph g ) {
        // construct the hierarchy, starting with existing types
        Iterator<GraphObject> iter = new DeepIterator( g, new TypeLabel() );
        while ( iter.hasNext() ) // for each type label in the type hierarchy graph
        {
            ...
            if ( mydescr.length == 0 ) {
            	com.conceptualgraph.charger.obj.Graph[] gs = Global.knowledgeManager.getAllGraphs();
                ...
                if ( mydescr.length == 0 ) // still no definition found
                {
                    ...
                    if ( mydescr.length == 0 ) {
                    	com.conceptualgraph.charger.Global.info( "no definition found for " + type.getTextLabel() );
                    }
                }
            }
            if ( mydescr.length > 0 ) {
     ...
     */
    private static void addNewTypeLabels( com.conceptualgraph.charger.obj.Graph g ) {
        Iterator<GraphObject> typelabeliter = new DeepIterator( g, new TypeLabel() );
        ...
        while ( gns.hasNext() ) {
            ...
            com.conceptualgraph.charger.obj.Concept c = (com.conceptualgraph.charger.obj.Concept)conceptVector.get( 2 );
        ...
        com.conceptualgraph.repgrid.tracks.TrackedRepertoryGrid[] grids = Global.knowledgeManager.getAllGrids();
     ...
     *
     */
    private static boolean okayToAddTypeLabel(com.conceptualgraph.charger.obj.Concept c, ArrayList typelabelvector ) {
    ...
    public static void linkTypeLabels( com.conceptualgraph.charger.obj.Graph h ) {
         ...
         */
        Iterator<GraphObject> typelabels = new DeepIterator( h, new TypeLabel() );
     ...
     */
    public static void linkHypernyms( TypeLabel t, Synset s, com.conceptualgraph.charger.obj.Graph h ) {
        //Craft.say( "linkHypernyms with type label " + t.getTypeLabel() );
        try {
            // find the set of hypernyms for that synset
            ...
            if ( pointers.length == 0 ) {
                ...
                for ( int p = 0; p < pointers.length; p++ ) {
                    //Craft.say( "hypernym " + p + " of " + pointers.length + " is " + pointers[ p ].getTarget() );
                    // see if any of the hypernyms are already used in type labels, 
                    ...
                    //Craft.say( "Found " + supertypes.length + " super-typelabels of " + pointers[p].getTarget() );
            ...
            //Craft.say( "hypernym tree is:" );
            //PointerUtils.getHypernymTree( answer ).print();
        } catch ( JWNLException je ) {
        	com.conceptualgraph.charger.Global.error( "Wordnet exception: " + je.getMessage() );
        }
     ...
     *
     * @see com.conceptualgraph.charger.Global#strs
    ...
    public static Action makeTypeHierarchyAction = new AbstractAction() {
        ...
        public void actionPerformed( ActionEvent e ) {
        	com.conceptualgraph.charger.obj.Graph g = makeTypeHierarchy();
            if ( g.getOwnerFrame() != null ) {
     ...
     */
    public static TypeLabel[] findBySynset( Synset s, com.conceptualgraph.charger.obj.Graph g ) {
