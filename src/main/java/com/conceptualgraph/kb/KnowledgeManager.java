 ...
 * Craft. A knowledge source is either a conceptual graph or a repertory grid.
 * This class is generally reserved for high level methods dealing with the
 * knowledge source management; most of the "lower level" detailed methods are
 * found in ConceptManager.
 *
 * @see KnowledgeSource
 * @see ConceptManager
 * @see com.conceptualgraph.kb.matching.AbstractMatchGroupMetrics
 */
public class KnowledgeManager {
    ...
    public Action summarizeKnowledgeAction = new AbstractAction() {
        public Object getValue( String s ) {
            if ( s.equals( Action.NAME ) ) {
                return com.conceptualgraph.charger.Global.strs( "SummarizeKnowledgeLabel" );
            }
        ...
        public void actionPerformed( ActionEvent e ) {
            ...
            //
            //Craft.say( "source tree of " + e.getActionCommand() + " is " );
            ...
            //Craft.say( "top level ancestor is " + top );
    ...
    public void performKnowledgeSummary( JFrame owner ) {
    	com.conceptualgraph.craft.Reporter.displaySummary( owner );
    }
    ...
    public void addKnowledgeSource( KnowledgeSource s ) {
        ...
        if ( s instanceof TrackedRepertoryGrid ) {
            gridCount++;
        }
        Craft.say( "added knowledge source number " + knowledgeSources.size() + " " + s.getClass() );
    }
    ...
    public void forgetKnowledgeSource( KnowledgeSource s ) {
        ...
        if ( Craft.craftModulePlugin != null ) {
            Craft.say( "removed knowledge source " + s.getClass()
    ...
    public static AbstractTypeDescriptor[] removeDuplicateDescriptors( AbstractTypeDescriptor[] ts ) {
        ...
        //Craft.say( "starting remove dups, ts has " + ts.length + " elements" );
        for ( int index = 0; index < v.size(); index++ ) {
            for ( int k = index + 1; k < v.size(); k++ ) {
                //Craft.say( "comparing index " + k + " with index " + index );
                if ( v.get( index ) != null && v.get( k ) != null ) {
                    ...
                    if ( o1.equals( o2 ) ) {
                        v.set( k, null );
                        //Craft.say( "deleting duplicate at index " + k );
                    }
                }
            }
        }
    ...
    public Graph[] getAllGraphs() {
        ...
        for ( int k = 0; k < knowledgeSources.size(); k++ ) {
            if ( knowledgeSources.get( k ) == null ) {
            	com.conceptualgraph.charger.Global.error( "Knowledge source number " + k + " was null." );
    ...
    /**
     * Locate any graphs that have type labels. Ideally there should be only one
     * hierarchy, but I'd rather not get into the one-size-fits-all argument
     * with the ontology group(s).
     *
     * @return a (possibly-empty) set of available knowledge source graphs that
     * contain at least one type label.
     */
     ...
     * of a tuple matcher. Any valid tuple matcher will work here, but usually
     * the name is chosen from a list in the Preferences window. It's assumed
     * that the matcher will be in the "com.conceptualgraph.kb" package, unless the name contains a
     * period (".")
     */
    public AbstractTupleMatcher createCurrentTupleMatcher() {
        String className = null;
        AbstractTupleMatcher matcher = null;
        className = com.conceptualgraph.charger.Global.matchingStrategy;
        if ( !className.contains( "." ) ) {
            className = "com.conceptualgraph.kb.matching." + className;
        }
        try {
            Class matcherClass = Class.forName( className );
            matcher = (AbstractTupleMatcher)matcherClass.newInstance();
        } catch ( ClassNotFoundException cnfe ) {
        	com.conceptualgraph.charger.Global.error( "Class " + className + " not found." );
