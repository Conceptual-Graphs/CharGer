...
public class RepertoryGrid { // extends JTable  {
     ...
     *
     * @see com.conceptualgraph.repgrid.tracks.TrackedRepertoryGrid
     */
...
//        }
    ...
    public RGCell getCell( RGAttribute a, RGElement e ) {
        //com.conceptualgraph.charger.Global.info( "getCell: attribute a is " + a.getLabel() + "; has " + a.getCells().size() + " cells");
        Iterator iter = a.getCells().iterator();
        while ( iter.hasNext() ) {
            RGCell c = (RGCell)iter.next();
            //com.conceptualgraph.charger.Global.info( "getCell: cell element is " + c.getElement() );
            if ( c.getElement() == e ) {
                return c;
            }
        }
        ...
        while ( iterelem.hasNext() ) {
            ...
            while ( iterattr.hasNext() ) {
                if ( !( valuetype instanceof RGBooleanValue ) ) {
                	com.conceptualgraph.charger.Global.error( "Can't export CXT on " + valuetype.getClass().getName() );
                    return null;
                } else {
