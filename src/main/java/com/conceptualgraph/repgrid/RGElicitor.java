...
import com.conceptualgraph.chargerlib.Transcript;
...
abstract public class RGElicitor {
    ...
    class MyDocumentListener implements DocumentListener {
        ...
        private void commitChanges( DocumentEvent e ) {
            ...
            /*window.validate();
             Craft.say( "after commit changes:" );
             try {
             Craft.say( "source of change is " + e.getDocument().getProperty( "name" ) + ": ("
     ...
     * @param m the table model for this grid; <code>null</code> if there isn't
     * any
     */
     ...
     *
     * @param w lets this elicitor know what window is controlling the component
     * it is handing out.
     ...
     * with which it was instantiated. Overridden in sub-classes to implement
     * particular elicitation strategies.
     */
    abstract public void elicit();

    /**
     * Looks for columns with identical values and queries for additional
     * attributes that can differentiate them. If the grid is not complete,
     * first calls <code>fillInGridByQuery</code> to complete it.
     */
    public void resolveSimilarities() {
        ...
        for ( int e1num = 0; e1num < elems.size(); e1num++ ) {
            ...
            for ( int e2num = e1num + 1; e2num < elems.size(); e2num++ ) // iterate through all the rest of the elements
            {
                ...
                if ( alike ) {
                    allDifferent = false;
                    Craft.say( grid.elementLabel + " \"" + e1.getLabel()
    ...
    protected void refreshHeaderPanel() {
        ...
        if ( grid.valuetype != null ) {
            valuetypeMenu.setEnabled( false );
            if ( grid.valuetype instanceof RGBooleanValue ) {
                valuetypeMenu.setSelectedItem( booleanchosen );
            } else if ( grid.valuetype instanceof RGIntegerValueRange ) {
                valuetypeMenu.setSelectedItem( range16chosen );
            }
    ...
    private void initializeHeaderPanel() {
        ...
        headerPanel.setBackground( Craft.craftPink );
        ...
        valuetypeMenu.addItemListener( new java.awt.event.ItemListener() {
            public void itemStateChanged( java.awt.event.ItemEvent e ) {
                if ( !valuetypeMenu.isEnabled() ) {
                    return;
                }
                String s = (String)valuetypeMenu.getSelectedItem();
                if ( s == null || s.equals( notypechosen ) ) {
                    return;
    ...
    /**
     * Queries the user for a new attribute in the repertory grid. If it's a
     * duplicate, a warning dialog appears. If it's valid, then it is added to
     * the repertory grid.
     *
     * @param s hopefully helpful string appended to the prompt.
     * @return the new attribute's label; <code>null</code> if user cancelled or
     * it's a duplicate.
     */
    public String queryNewAttribute( String s ) {
        String newAttr = transcript.showInputDialog(window,
                "Enter " + General.a_or_an( grid.attributeLabel ) + s + ":" );
        if ( newAttr != null ) {
    ...
    /**
     * Queries the user for a new element in the repertory grid. If it's a
     * duplicate, a warning dialog appears. If it's valid, then it is added to
     * the repertory grid.
     *
     * @param s hopefully helpful string appended to the prompt.
     * @return the new element's label; <code>null</code> if user cancelled or
     * it's a duplicate.
     */
    public String queryNewElement( String s ) {
        String newElem = transcript.showInputDialog(window,
                "Enter example of " + General.a_or_an( grid.elementLabel ) + s + ":" );
        if ( newElem != null ) {
