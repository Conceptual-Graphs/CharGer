 ...
 */
public class TrackedRepertoryGrid extends RepertoryGrid implements com.conceptualgraph.kb.KnowledgeSource {
	...
	public ObjectLocator conceptOneLocator = null;
		/** Locator for the concept corresponding to the attribute aspect
			@see com.conceptualgraph.charger.util.ObjectLocator */
	public ObjectLocator conceptTwoLocator = null;
		/** Locator for either a relation or an actor
			@see com.conceptualgraph.charger.util.ObjectLocator */
	public ObjectLocator relationLocator = null;
...
/*	public TypeDescriptor getTypeDescriptor( String label )
	...
	*/
	 ...
	 */
	 ...
	 {
				//com.conceptualgraph.charger.Global.info( "calling replace attr; old has " + oldattr.getCells().size() + " cells." );
		int anum = 0;
		while ( anum < attributes.size() && attributes.get( anum ) != oldattr ) 
			anum++;
					//com.conceptualgraph.charger.Global.info( "attr num to replace is " + anum + " of " + attributes.size() + " total.");
	...
	public com.conceptualgraph.charger.gloss.AbstractTypeDescriptor[] findTypeDescriptors(String term )
	{
		ArrayList holder = new ArrayList();
		AbstractTypeDescriptor[] ds = getAllTypeDescriptors();
					Craft.say( "grid has " + ds.length + " type descriptors, looking for term " + term );
		for ( int k = 0; k < ds.length; k++ )
		{
					//Craft.say( "descriptor " + k + " has label " + ds[ k ].getLabel() );
			if ( term.equalsIgnoreCase( ds[ k ].getLabel() ) )
			{
				holder.add( ds[ k ] );
						//Craft.say( "found a descriptor to match the term " + term );
			}
		}
	 ...
	 */
	public com.conceptualgraph.charger.obj.Graph[] getAllGraphs()
	{
