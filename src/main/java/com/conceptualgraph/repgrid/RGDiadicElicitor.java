...
public class RGDiadicElicitor extends RGElicitor {
    ...
    public void elicit() {
        ...
        if ( grid.elementLabel == null || grid.relationLabel == null || grid.attributeLabel == null
            ...
            while ( okay ) {
                ...
                if ( okay ) {
                    okay = nameTwoAttributes();
                }
                if ( okay ) {
                    okay = fillInGridByQuery();
                }
            }
        }
    }
    ...
    public boolean nameTwoAttributes() {
        String t2 = null;
        String t1 = transcript.showInputDialog( null, "Name " + General.a_or_an( grid.attributeLabel )
                + " that " + lastElements.get( 0 ) + " and "
                + lastElements.get( 1 ) + " both " + com.conceptualgraph.craft.Reporter.tmap.cardinalize( grid.relationLabel, 1 ) );
        ...
        if ( t1 == null ) {
            ...
            //if ( model != null ) model.fireTableStructureChanged();
            t2 = transcript.showInputDialog( null, "Name " + General.a_or_an( grid.attributeLabel )
