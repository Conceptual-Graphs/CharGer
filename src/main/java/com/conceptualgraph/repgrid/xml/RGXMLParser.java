...
//
package com.conceptualgraph.repgrid.xml;

import org.xml.sax.helpers.*;

import com.conceptualgraph.charger.Global;
import com.conceptualgraph.charger.util.*;
 ...
 *
 * @see com.conceptualgraph.repgrid.xml.RGXMLGenerator
 */
public class RGXMLParser extends DefaultHandler {
    ...
    public TrackedRepertoryGrid parseXMLRepertoryGridFile( InputStreamReader instream, File f ) {
        ...
        Global.info( "ready to parse... grid is " + grid );
...
//        XMLReader xr = new org.apache.crimson.parser.XMLReaderImpl();
        ...
        try {
            ...
            Global.error( "IO Exception in XMLParser: " + ioe.getMessage() );
        } catch ( SAXException se ) {
            Exception e = se.getException();
            if ( e == null ) {
                e = se;
            }
            Global.error( "Sax Exception in RGXMLParser: " + e.getMessage() );
        }
        return grid;
    }
    ...
    public void startDocument() {
        if ( traceTags ) {
            Craft.say( "Start document; grid is " + grid );
        }
    ...
    public void startElement( String uri, String name,
        ...
        if ( qName.equals( "cell" ) ) {
            try {
                ...
                Global.error( "RGValueException: " + ve.getMessage() );
            }
        }
        if ( qName.equals( "element" ) ) {
           ...
           ObjectLocator loc = null;
            ...
            if ( atts.getValue( "uri" ) != null ) {
                loc =ObjectLocator.parseURI( atts.getValue( "uri" ) );
            }
            //Craft.say( "header label type is " + headertype +  "; text is " + text + "; uri is " + atts.getValue( "uri" ) );
            if ( headertype.equals( "element" ) ) {
                grid.setElementLabel( text );
                grid.conceptOneLocator = loc;
                if ( loc != null ) {
                    grid.conceptOne = (com.conceptualgraph.charger.obj.Concept)loc.getObject( true );
                }
            } else if ( headertype.equals( "relation" ) ) {
                grid.setRelationLabel( text );
                grid.relationLocator = loc;
                if ( loc != null ) {
                    grid.relation = (com.conceptualgraph.charger.obj.GNode)loc.getObject( true );
                }
            } else if ( headertype.equals( "attribute" ) ) {
                grid.setAttributeLabel( text );
                grid.conceptTwoLocator = loc;
                if ( loc != null ) {
                    grid.conceptTwo = (com.conceptualgraph.charger.obj.Concept)loc.getObject( true );
                }
            ...
            if ( qName.equals( "wordnet" ) ) {
                wordnetStack.push( com.conceptualgraph.charger.gloss.wn.WordnetTypeDescriptor.getInstanceFromXML( atts ) );
            } else if ( qName.equals( "generic" ) ) {
                genericStack.push( com.conceptualgraph.charger.gloss.GenericTypeDescriptor.getInstanceFromXML( atts ) );
            }
    ...
    public void endElement( String uri, String name, String qName ) {
        ...
        if ( qName.equals( "string" ) ) // Not used here
        {
            ...
            grid.addElement( genericString, vectorInsertionPoint );
        } else if ( qName.equals( "attribute" ) ) {
            ...
            {
                ( (com.conceptualgraph.charger.gloss.wn.WordnetTypeDescriptor)wordnetStack.peek() ).setLabel( (String)labelStack.pop() );
                genericString = "";
            } else if ( !genericStack.isEmpty() ) {
                ( (com.conceptualgraph.charger.gloss.GenericTypeDescriptor)genericStack.peek() ).setLabel( (String)labelStack.pop() );
                genericString = "";
            ...
            if ( !wordnetStack.isEmpty() ) {
                ( (com.conceptualgraph.charger.gloss.wn.WordnetTypeDescriptor)wordnetStack.peek() ).setDefinition( genericString );
                //Craft.say( "setting wordnet definition: " + genericString );
                genericString = "";
            } else if ( !genericStack.isEmpty() ) {
                ( (com.conceptualgraph.charger.gloss.GenericTypeDescriptor)genericStack.peek() ).setDefinition( genericString );
                //Craft.say( "setting generic definition: " + genericString );
                genericString = "";
            }
            ...
            ( (TrackedAttribute)currentAttribute ).setTypeDescriptor(
                    ( (com.conceptualgraph.charger.gloss.wn.WordnetTypeDescriptor)wordnetStack.peek() ).getLabel(),
                    (com.conceptualgraph.charger.gloss.AbstractTypeDescriptor)wordnetStack.pop() );
        } else if ( qName.equals( "generic" ) ) {
            // assumes that the first generic start tag caused us to switch to a tracked attribute
            ( (TrackedAttribute)currentAttribute ).setTypeDescriptor(
                    ( (com.conceptualgraph.charger.gloss.GenericTypeDescriptor)genericStack.peek() ).getLabel(),
                    (com.conceptualgraph.charger.gloss.AbstractTypeDescriptor)genericStack.pop() );
        }
    }
